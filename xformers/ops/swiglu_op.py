...
@register_operator
class DualGemmOp(BaseOperator):
    OPERATOR = get_xformers_operator("dual_gemm")
    OPERATOR_CATEGORY = "swiglu"
    NAME = "dual_gemm"
...
@register_operator
class SiluIdentityMulGemmOp(BaseOperator):
    OPERATOR = get_xformers_operator("silu_identity_mul_gemm")
    OPERATOR_CATEGORY = "swiglu"
    NAME = "silu_identity_mul_gemm"

    @classmethod
    # type: ignore
    def operator_flop(cls, gx: torch.Tensor, ux: torch.Tensor, w2: torch.Tensor) -> int:
        M, N, K = gx.shape[0], w2.shape[0], gx.shape[1]
        return M * N * K * 2
...
class _SwiGLUFusedFuncNew(torch.autograd.Function):
    ...
    @torch.cuda.amp.custom_fwd
    def forward(cls, ctx, x, w1, b1, w2, b2, w3, b3):
        x1, x2 = DualGemmOp.OPERATOR(x, w1, b1, w2, b2)
        x5 = SiluIdentityMulGemmOp.OPERATOR(x1, x2, w3, b3)
        ctx.save_for_backward(x, w1, w2, w3, x1, x2)
        ctx.bias = [b1 is not None, b2 is not None, b3 is not None]
        return x5
...
class SwiGLUOpNew:
    ...
    def supports(self, op: "SwiGLUOpNewDispatch") -> bool:
        if self.PACKED_WEIGHTS and not op.packed_weights:
            return False
        return all(c(op) for c in self.constraints)
    ...
    def __str__(self) -> str:
        return f"SwiGLUOpNew:{self.NAME}"


class _ForwardToPythonAutogradFuncNew(SwiGLUOpNew):
    def supports(self, op: "SwiGLUOpNewDispatch") -> bool:
        # Let's disable autocast in bf16 until this issue is fixed
        # https://github.com/pytorch/pytorch/issues/87979
        if op.dtype_autocast_gpu == torch.bfloat16:
...
class _ForwardToFuncNew(SwiGLUOpNew):
    def __call__(self, *args, **kwargs):
        return self.op(*args, **kwargs)
...
@dataclass
class SwiGLUOpNewDispatch:
    """Dispatcher to automatically select
    the best operator in :attr:`xformers.ops.swiglu_new`
    """
    ...
    def op(self) -> SwiGLUOpNew:
        ...
        for i, op in enumerate(priorities):
            if op.supports(self):
                # print (f"{type(op).__name__}: {i}\n")
                return op
        # print (f"{type(SwiGLUEagerOp).__name__}: 999\n")
        return SwiGLUEagerOp
...
def _only_sm80_new(op: SwiGLUOpNewDispatch) -> bool:
    device_type = op.device if isinstance(op.device, str) else op.device.type
    return device_type == "cuda" and torch.cuda.get_device_capability(op.device)[0] >= 8


def _only_half_or_autocast_new(op: SwiGLUOpNewDispatch) -> bool:
    HALF_DTYPES = [torch.half, torch.bfloat16]
    return op.dtype in HALF_DTYPES or (
        op.dtype_autocast_gpu is not None and op.dtype_autocast_gpu in HALF_DTYPES
    )
...
SwiGLUFusedOpNew = _ForwardToPythonAutogradFuncNew(
    _SwiGLUFusedFuncNew, False, "fused_new", constraints=[_only_sm80_new, _only_half_or_autocast_new]
)
...
SwiGLUPackedFusedOpNew = _ForwardToFuncNew(
    get_xformers_operator("swiglu_packedw_new"),
    True,
    "fused_new.p.cpp",
    constraints=[_only_sm80_new, _only_half_or_autocast_new],
)
...
class SwiGLU(nn.Module):
    ...
    Fuse the first two linear layers, silu and mul into a single operator.
    """
...
def swiglu_new(
    ...
    *,
    ...
    if op is None:
        op = SwiGLUOpNewDispatch.from_arguments(x, w1, b1, w2, b2, w3, b3).op
...
class SwiGLUNew(nn.Module):
    """
    A Module that encapsulates the call to :attr:`xformers.ops.swiglu_new`,
    and holds the weights for the 3 linear layers.
    Fuse silu, mul and the last linear layer into a single operator.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.out_features = out_features
        self.in_features = in_features
        self.op: Optional[SwiGLUOpNew] = None
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.w12 is not None:
            if self.op is not None:
                ...
                return swiglu_packed_new(x, *self._packed_ordered_params(), op=self.op)

        return swiglu_new(x, *self._ordered_params(), op=self.op)
